...
def test_config():
    ...
    try:
        ...
        print(f"[OK] 配置读取成功 - 商品ID: {sku_id}, 购买时间: {buy_time}")
        return True
    except Exception as e:
...
def test_timer():
    ...
    try:
        from maotai.timer import Timer
        timer = Timer()
        print(f"[OK] Timer初始化成功 - 购买时间: {timer.buy_time}")
        print(f"[OK] 时间差: {timer.diff_time}ms")
        return True
    except Exception as e:
        print(f"[ERROR] Timer初始化失败: {e}")
        return False
...
def test_jd_seckill():
    ...
    try:
        from maotai.jd_spider_requests import JdSeckill
        jd = JdSeckill()
        print(f"[OK] JdSeckill初始化成功 - 商品ID: {jd.sku_id}")
        return True
    except Exception as e:
        print(f"[ERROR] JdSeckill初始化失败: {e}")
        return False
