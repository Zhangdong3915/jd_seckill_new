...
def test_device_fingerprint():
    ...
    try:
        ...
        has_eid_quotes = current_eid and ('"' in current_eid or "'" in current_eid)
        has_fp_quotes = current_fp and ('"' in current_fp or "'" in current_fp)
        print(f"eid是否包含引号: {'是' if has_eid_quotes else '否'}")
        print(f"fp是否包含引号: {'是' if has_fp_quotes else '否'}")
