...
class DeviceFingerprintCollector:
    ...
    def _generate_eid(self):
        ...
        random_part = ''.join(random.choice(chars) for _ in range(60))

        # 生成基于机器特征的部分
        machine_info = f"{timestamp}{random_part}"
        hash_part = hashlib.md5(machine_info.encode()).hexdigest().upper()[:20]

        return f"JD_Cartmain_{timestamp}_{hash_part}_{random_part}"
