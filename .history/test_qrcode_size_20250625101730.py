...
def test_qrcode_size():
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # 检查文件大小（更大的二维码文件应该更大）
            file_size = os.path.getsize(test_qr_file)
            print(f"📏 二维码文件大小: {file_size} 字节")

            # 验证文件大小是否合理（294px的二维码应该比147px的大）
            # 一般来说，294px的二维码文件大小应该在3KB以上
            if file_size > 3000:  # 3KB
                ...
                try:
                    if os.name == "nt":  # Windows
                        os.system(f'start {test_qr_file}')
                        print("🖼️ 二维码图片已打开，请目视确认尺寸")
                except:
                    pass

                return True
            else:
                print("❌ 文件大小异常，可能尺寸调整未生效")
                print(f"   文件大小: {file_size} 字节")
                return False
