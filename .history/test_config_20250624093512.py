...
def test_config():
    ...
    try:
        ...
        print(f"✅ 抢购数量: {seckill_num}")
        print(f"✅ 抢购时间: {buy_time}")
        
        # 测试账户配置
        try:
            payment_pwd = global_config.getRaw('account', 'payment_pwd')
            print(f"✅ 支付密码: {'已配置' if payment_pwd else '未配置'}")
        except:
            print("✅ 支付密码: 未配置")
        ...
        try:
            ...
            print(f"✅ 微信通知: {'启用' if enable == 'true' else '禁用'}")
            print(f"✅ SCKEY: {'已配置' if sckey else '未配置'}")
        except:
            print("✅ 微信通知: 禁用")
        
        print("\n🎉 配置文件格式正确，所有配置项都能正常读取！")
        return True
