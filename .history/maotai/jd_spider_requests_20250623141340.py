...
class QrLogin:
    ...
    def _complete_login(self):
        ...
        try:
            # 访问登录完成页面
            complete_url = 'https://passport.jd.com/uc/login?ltype=logout'
            headers = {
                'User-Agent': self.spider_session.get_user_agent(),
                'Referer': 'https://passport.jd.com/new/login.aspx',
            }

            resp = self.session.get(url=complete_url, headers=headers, allow_redirects=True)
            logger.info(f'登录完成页面访问状态: {resp.status_code}')
