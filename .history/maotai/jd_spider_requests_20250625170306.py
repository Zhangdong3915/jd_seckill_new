...
class JdSeckill(object):
    ...
    def make_reserve(self):
        ...
        print("3. 预约时间：工作日10:00-12:00")
        print("4. 建议预约时间：工作日10:05左右")
        print("="*60)
        print("请使用以下方式进行预约：")
        print("📱 打开京东APP")
        ...
        return self._check_reserve_status()
    ...
    def _check_reserve_status(self):
        ...
        try:
            ...
            # 检查是否是404或错误页面
            error_indicators = [
                '页面不存在', '页面未找到', '404', 'not found',
                '您所访问的页面不存在', '页面走丢了', '找不到页面'
            ]

            # 检查是否真的需要登录
            login_indicators = [
                '登录', 'login', '请先登录', '未登录',
                'passport.jd.com', '账户登录', '扫码登录'
            ]

            # 检查预约相关的正常状态
            normal_indicators = [
                '预约成功', '已预约', '预约期已过', '商品已下架',
                '不在预约时间', '预约未开始', '预约已结束'
            ]

            # 判断页面类型
            is_error_page = any(indicator in resp_text_lower for indicator in [i.lower() for i in error_indicators])
            needs_login = any(indicator in resp_text_lower for indicator in [i.lower() for i in login_indicators])
            is_normal_status = any(indicator in resp_text_lower for indicator in [i.lower() for i in normal_indicators])

            print(f"🔍 页面分析结果:")
            print(f"   是否错误页面: {is_error_page}")
            print(f"   是否需要登录: {needs_login}")
            print(f"   是否正常状态: {is_normal_status}")

            if is_error_page:
                print("\n" + "="*60)
                print("❌ 预约接口错误")
                print("="*60)
                print("预约接口返回404错误页面，可能原因：")
                print("1. 预约接口URL已失效或变更")
                print("2. 商品ID不正确")
                print("3. 当前不在预约时间范围内")
                print("4. 商品已下架或停止预约")
                print("="*60)
                logger.error('预约接口返回404错误页面')
                raise Exception('预约接口不可用：页面不存在(404)')
            elif needs_login and not is_normal_status:
                print("\n" + "="*60)
                print("🔐 登录已过期")
                print("="*60)
                ...
                if 'text/html' in content_type and any(indicator in resp.text.lower() for indicator in [i.lower() for i in login_indicators]):
                    raise Exception('重新登录后预约接口仍然失效')
            elif is_normal_status:
                logger.info('预约接口返回正常状态页面，可能是预约期外或已预约')
                # 尝试从HTML中提取有用信息
                if '预约成功' in resp.text or '已预约' in resp.text:
                    logger.info('检测到已预约成功的状态')
                    return
                elif '预约期已过' in resp.text or '预约未开始' in resp.text:
                    logger.warning('当前不在预约时间范围内')
                    return
                else:
                    logger.warning('预约页面状态未知，尝试继续解析')
            else:
                logger.warning('预约接口返回HTML页面，但未检测到明确的登录或状态信息')
                # 不立即抛出异常，尝试解析内容
        ...
        # 同时记录到日志
        logger.info(f'预约接口响应长度: {len(resp.text)}')
        logger.info(f'预约接口响应前1000字符: {resp.text[:1000]}')
        ...
        try:
            ...
            logger.error(f'响应内容前500字符: {resp.text[:500]}')

            # 如果是HTML响应，尝试从中提取有用信息
            if 'text/html' in content_type:
                logger.info('尝试从HTML响应中提取预约状态信息...')

                # 检查常见的预约状态
                if '预约成功' in resp.text or '已预约' in resp.text:
                    logger.info('从HTML中检测到预约成功状态')
                    print("✅ 预约成功！（从HTML页面检测）")
                    return
                elif '预约期已过' in resp.text or '预约未开始' in resp.text:
                    logger.warning('从HTML中检测到预约期外状态')
                    print("⚠️ 当前不在预约时间内")
                    return
                elif '商品已下架' in resp.text:
                    logger.warning('从HTML中检测到商品已下架')
                    print("⚠️ 商品已下架")
                    return
                else:
                    logger.error('HTML响应中未找到明确的预约状态信息')
