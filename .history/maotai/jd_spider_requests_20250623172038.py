...
class JdSeckill(object):
    ...
    def get_secure_payment_password(self, required=True):
        ...
        if self.secure_config:
            ...
            # 备用方案：从配置文件直接读取
            password = global_config.getRaw('account', 'payment_pwd', fallback='')
    ...
    def get_secure_sckey(self, required=True):
        ...
        if self.secure_config:
            ...
            # 备用方案：从配置文件直接读取
            sckey = global_config.getRaw('messenger', 'sckey', fallback='')
    ...
    def validate_required_config(self):
        ...
        # 检查微信通知配置（条件必需）
        messenger_enable = global_config.getRaw('messenger', 'enable', fallback='false')
