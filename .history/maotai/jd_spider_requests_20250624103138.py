...
class JdSeckill(object):
    ...
    def get_time_status(self):
        ...
        # 预约时间：11:00
        # 抢购时间：12:00-12:30
        reserve_time = datetime.combine(now.date(), datetime.strptime("11:00:00.000", "%H:%M:%S.%f").time())
        ...
        if now > last_purchase_time:
            ...
            if tomorrow.weekday() < 5:  # 明天是工作日
                reserve_time = datetime.combine(tomorrow, datetime.strptime("11:00:00.000", "%H:%M:%S.%f").time())
                ...
                reserve_time = datetime.combine(next_workday, datetime.strptime("11:00:00.000", "%H:%M:%S.%f").time())
        ...
        if now < reserve_time:  # 还没到预约时间
            return {
                ...
                'description': f'距离预约时间(11:00)还有 {int(time_to_reserve//3600)}小时{int((time_to_reserve%3600)//60)}分钟'
            }
        elif now < buy_time:  # 预约时间段（11:00-12:00）
            return {
                'status': 'reserve_time',
                'action': '执行预约',
            ...
            while (now.date() + timedelta(days=days_to_add)).weekday() >= 5:
                days_to_add += 1
            next_workday = now.date() + timedelta(days=days_to_add)
            next_reserve_time = datetime.combine(next_workday, datetime.strptime("11:00:00.000", "%H:%M:%S.%f").time())
            ...
            return {
                ...
                'description': f'今日抢购已结束，等待下个工作日11:00预约'
            }
