...
class QrLogin:
    ...
    def _complete_login(self):
        ...
        try:
            # 尝试多个可能的登录完成URL
            complete_urls = [
                'https://passport.jd.com/uc/login?ltype=logout&ReturnUrl=https://www.jd.com/',
                'https://passport.jd.com/new/login.aspx?ReturnUrl=https://www.jd.com/',
                'https://www.jd.com/',
            ...
            for url in complete_urls:
                try:
                    ...
                    resp = self.session.get(url=url, headers=headers, allow_redirects=True, timeout=10)
                    logger.info(f'访问状态: {resp.status_code}')

                    # 检查是否获取到了关键Cookie
                    cookies = self.session.cookies
                    if 'pt_key' in cookies or 'pt_pin' in cookies:
                        logger.info(f'在访问 {url} 后获取到关键Cookie')
                        break
            ...
            # 最终检查Cookie状态
            cookie_count = len(self.session.cookies)
            logger.info(f'登录完成后Cookie数量: {cookie_count}')

            # 列出所有Cookie名称
            cookie_names = [cookie.name for cookie in self.session.cookies]
            logger.info(f'登录完成后Cookie列表: {cookie_names}')

            # 检查关键Cookie
            key_cookies = ['pt_key', 'pt_pin', 'pwdt_id']
            for cookie_name in key_cookies:
                if cookie_name in self.session.cookies:
                    logger.info(f'成功获取关键Cookie: {cookie_name}')
                else:
                    logger.warning(f'仍然缺少关键Cookie: {cookie_name}')
